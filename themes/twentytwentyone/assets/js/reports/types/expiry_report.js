jQuery(document).ready(function($) {
    ...
    const VAL     = EXPIRY_REPORT_VAL;

    const table = $('#expiry_reportsTable').DataTable({
        columns: [
            ...
            { title: "User that Activated" },
            { title: "Activation Date Missed (Y/N)" },
            { title: "Card Expiry Date" },
            { title: "Expiry Date Past (Y/N)" },
            { title: "Expired" },
            { title: "Supplier" },
            { title: "Admin Activated (Y/N)" },
            { title: "Admin User Activated" },
        ],
                ...
                exportOptions: {
                    ...
                    format: {
                        header: function (data, columnIdx) {
                            return $('#expiry_reportsTable thead th').eq(columnIdx).clone().children().remove().end().text().trim();
                        }
                    }
                }
            },
    ...
    jQuery('#expiry_reportsTable thead th').each(function (index) {
    ...
    jQuery('#expiry_reportsTable thead').on('click', '.filter-icon', function (e) {
        e.stopPropagation();
    ...
    jQuery('#expiry_reportsTable thead').on('change', '.status-filter', function () {
    ...
    // Live filtering
    jQuery('#expiry_reportsTable thead').on('keyup change', '.column-search', function () {
        ...
        if (colIndex === 0) { // Date column: handle range filter
            ...
            if (fromDate || toDate) {
                $.fn.dataTable.ext.search.push(function dateRangeFilter(settings, data, dataIndex) {
                    if (settings.nTable.id !== 'expiry_reportsTable') return true;
        ...
        const isLastColumn = colIndex === $('#expiry_reportsTable thead th').length - 1;
    ...
    async function fetchGiftCards(page = 1, perPage = 50) {
        try {
            ...
            if (EXPIRY_REPORT_VAL.permissions != 1) throw new Error('Not allowed');
    ...
    async function loadGiftCardsForTable() {
        ...
        table.rows.add(firstPage.giftcards.map(gc => [
            ...
            gc.activated_y_n,
            gc.date_activated,
            gc.user_that_activated,
            gc.activation_date_missed_y_n,
            gc.card_expiry_date,
            gc.expiry_date_past_y_n,
            gc.expired,
            gc.supplier,
            gc.admin_activated_y_n,
            gc.admin_user_activated,
        ])).draw();
