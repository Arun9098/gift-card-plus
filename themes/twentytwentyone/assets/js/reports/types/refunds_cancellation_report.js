jQuery(document).ready(function($) {
    ...
    const tableId = 'refunds_cancellation_reportTable';
    const VAL     = REFUNDS_CANCELLATION_REPORT_VAL;
    ...
    $('#' + tableId + ' thead th').each(function(index) {
        ...
        if (isDate) {
            ...
            inputField = `<div class="status-checkboxes" data-col="${index}">
                <label style="display:block; margin-bottom:3px;"><input type="checkbox" class="rcr-status-filter" value="Refunded"> Refunded</label>
                <label style="display:block; margin-bottom:3px;"><input type="checkbox" class="rcr-status-filter" value="Cancelled"> Cancelled</label>
            </div>`;
        } else {
    ...
    $('#' + tableId + ' thead').on('change', '.rcr-status-filter', function() {
        const selected = $('.rcr-status-filter:checked').map(function() {
    ...
    $('#' + tableId + ' thead').on('keyup change', '.column-search', function() {
        ...
        if (colIndex === 0) {
            ...
            $.fn.dataTable.ext.search = $.fn.dataTable.ext.search.filter(fn => fn.name !== 'rcrDateFilter');
            if (from || to) {
                $.fn.dataTable.ext.search.push(function rcrDateFilter(settings, data) {
    ...
    async function loadOrdersForTable() {
        ...
        table.clear();
        table.rows.add(first.orders.map(rcr_data)).draw();
        const totalPages = Math.ceil(first.total_orders / 50);
        for (let p = 2; p <= totalPages; p++) {
            const page = await fetchOrders(p, 50);
            table.rows.add(page.orders.map(rcr_data)).draw(false);
        }
    }
