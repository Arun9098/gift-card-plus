jQuery(document).ready(function($) {
    ...
    const VAL     = USER_REPORT_VAL;

    const table = $('#user_reportsTable').DataTable({
        columns: [
            { title: "User ID" },
            { title: "Status" },
            { title: "User Type" },
                ...
                exportOptions: {
                    ...
                    format: {
                        header: function (data, columnIdx) {
                            return $('#user_reportsTable thead th').eq(columnIdx).clone().children().remove().end().text().trim();
                        }
                    }
                }
            },
    ...
    jQuery('#user_reportsTable thead th').each(function (index) {
        ...
        if (1==2 && isDateColumn) {
            ...
            <label style="display:block; margin-bottom:3px;">
                        <input type="checkbox" name="o_status" class="status-filter" value="Business"> Business
                    </label>
                    <label style="display:block; margin-bottom:3px;">
                        <input type="checkbox" name="o_status" class="status-filter" value="Consumer"> Consumer
                    </label>
            </div>`;
        } else {
    ...
    jQuery('#user_reportsTable thead').on('click', '.filter-icon', function (e) {
        e.stopPropagation();
    ...
    jQuery('#user_reportsTable thead').on('change', '.status-filter', function () {
    ...
    // Live filtering
    jQuery('#user_reportsTable thead').on('keyup change', '.column-search', function () {
        ...
        if (1==2 && colIndex === 1) { // Date column: handle range filter
            ...
            if (fromDate || toDate) {
                $.fn.dataTable.ext.search.push(function dateRangeFilter(settings, data, dataIndex) {
                    if (settings.nTable.id !== 'user_reportsTable') return true;
        ...
        const isLastColumn = colIndex === $('#user_reportsTable thead th').length - 1;
    ...
    async function fetchUsers(page = 1, perPage = 50) {
        try {
            ...
            if (USER_REPORT_VAL.permissions != 1) throw new Error('Not allowed');
