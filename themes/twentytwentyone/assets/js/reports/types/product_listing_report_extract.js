jQuery(document).ready(function($) {
    ...
    const VAL     = PRODUCT_LISTING_REPORT_EXTRACT_VAL;

    const table = $('#product_listing_report_extractsTable').DataTable({
        columns: [
            { title: "Product ID" },
                ...
                exportOptions: {
                    ...
                    format: {
                        header: function (data, columnIdx) {
                            return $('#product_listing_report_extractsTable thead th').eq(columnIdx).clone().children().remove().end().text().trim();
                        }
                    }
                }
            },
    ...
    jQuery('#product_listing_report_extractsTable thead th').each(function (index) {
    ...
    jQuery('#product_listing_report_extractsTable thead').on('click', '.filter-icon', function (e) {
        e.stopPropagation();
    ...
    jQuery('#product_listing_report_extractsTable thead').on('change', '.status-filter', function () {
    ...
    // Live filtering
    jQuery('#product_listing_report_extractsTable thead').on('keyup change', '.column-search', function () {
        ...
        if (1==2 && colIndex === 1) { // Date column: handle range filter
            ...
            if (fromDate || toDate) {
                $.fn.dataTable.ext.search.push(function dateRangeFilter(settings, data, dataIndex) {
                    if (settings.nTable.id !== 'product_listing_report_extractsTable') return true;
        ...
        const isLastColumn = colIndex === $('#product_listing_report_extractsTable thead th').length - 1;
    ...
    async function fetchProducts(page = 1, perPage = 50) {
        try {
            ...
            if (PRODUCT_LISTING_REPORT_EXTRACT_VAL.permissions != 1) throw new Error('Not allowed');
