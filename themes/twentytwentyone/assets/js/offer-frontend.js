jQuery(document).ready(function($) {
    ...
    function handleFileUpload(file) {
        ...
        reader.onload = function(e) {
            ...
            if (!base64Data || base64Data.length < 100) {
                alert('Error: Image data is invalid. Please try again.');
                resetImageBox();
                return;
            }
            
            // Store base64 data and filename in hidden fields (temporary - not saved to media)
            // Note: Large base64 strings might be truncated in form submission
    ...
    const $select = $('#offer_showcase_type');
    const $promoField = $('#offer_promo_code');
    const $linkField = $('#offer_link');

    function toggleRequired() {
        var value = $select.val();

        // Reset first
        $promoField.prop('required', false);
        $linkField.prop('required', false);

        if (value === 'promo_code' || value === 'copy') {
            $promoField.prop('required', true);
        } else if (value === 'link') {
            // Only require single link field when bulk mode is NOT active
            var bulkActive = $('#bulk-links-wrapper').is(':visible');
            if (!bulkActive) {
                $linkField.prop('required', true);
            }
        }
    }

    // On change
    $select.on('change', toggleRequired);

    // Run once on page load (important for pre-selected value)
    toggleRequired();
    
    $('#offer-form').on('keydown', function (e) {
        if (e.key === 'Enter' && !$(e.target).is('textarea')) {
            e.preventDefault();

            var $form = $(this);

            // jQuery's trigger('submit') below skips native HTML5 validation
            // (required fields etc.), unlike a real click on a submit button.
            // Run it manually so Enter-to-submit can't bypass required fields.
            if (this.reportValidity && !this.reportValidity()) {
                return;
            }

            var status = $form.find('[name="offer_status"]').val();

            // Remove previously added hidden inputs
            $form.find('input[name="update"], input[name="save_as_draft"], input[name="save_and_publish"]').remove();
    ...
    $('#offer-form').on('submit', function(e) {
        ...
        // ── BULK LINKS: pack into offer_link BEFORE image upload or native submit ──
        packBulkLinksIntoField();
        // ── END BULK LINKS ──
    ...
    // "Available for all products" ↔ individual product selection are mutually exclusive
    var $allProductsCheckbox = $('#offer_all_products');
    var $productSearchInput  = $('#product-search');
    var $addBulkBtn          = $('#add-bulk-btn');

    function toggleProductSelectionUI() {
        var isAllProducts = $allProductsCheckbox.prop('checked');

        if (isAllProducts) {
            // Clear all individually selected products
            $('#selected-products-list .selected-product-item').each(function () {
                var pid = $(this).data('product-id');
                delete selectedProducts[pid];
                $('#products-table-body tr[data-product-id="' + pid + '"]').remove();
            });
            $('#selected-products-list').html('<p class="no-products-message">No products selected. Search and add products below.</p>');

            // Disable search + add bulk
            $productSearchInput.prop('disabled', true).attr('placeholder', 'Disabled — "Available for all products" is checked');
            $addBulkBtn.prop('disabled', true);
        } else {
            $productSearchInput.prop('disabled', false).attr('placeholder', 'Search product title, SKU, brand or category');
            $addBulkBtn.prop('disabled', false);
        }
    }

    // When checkbox changes
    $allProductsCheckbox.on('change', toggleProductSelectionUI);

    // When a product is added individually → uncheck "all products"
    $(document).on('click', '.btn-add-product', function () {
        if ($allProductsCheckbox.prop('checked')) {
            $allProductsCheckbox.prop('checked', false);
            toggleProductSelectionUI();
        }
    });

    // Run once on page load to apply correct state if checkbox is pre-checked
    toggleProductSelectionUI();
    ...
    var $bulkWrapper     = $('#bulk-links-wrapper');
    var $singleWrapper   = $('#single-link-wrapper');
    var $bulkList        = $('#bulk-links-list');
    var $csvInput        = $('#bulk-links-csv-input');
    var $csvHint         = $('#bulk-links-csv-hint');
    var $csvError        = $('#bulk-links-error');
    var $addAnotherBtn   = $('#add-another-link-btn');
    var $reUploadBtn     = $('#re-upload-csv-btn');
    var $bulkCountLabel  = $('#bulk-links-count');
    var $singleLinkInput = $('#offer_link');

    // On page load — if PHP passed saved bulk links, populate rows and switch to bulk mode
    if (typeof offerBulkLinks !== 'undefined' && offerBulkLinks.length > 0) {
        populateBulkLinks(offerBulkLinks);
        enterBulkMode();
    } else if ($bulkWrapper.is(':visible')) {
        // Fallback: bulk wrapper already visible (server-side rendered rows)
        enterBulkMode();
        updateBulkCount();
    }

    // "Add Bulk Offer Links" button → open CSV picker
    $('#add-bulk-links-btn').on('click', function () {
        clearBulkError();
        $csvInput.val('');
    ...
    // CSV file selected → parse and populate
    $csvInput.on('change', function () {
        ...
        var reader = new FileReader();
        reader.onload = function (e) {
            var links = parseBulkCsv(e.target.result);

            if (links.length === 0) {
                showBulkError('No valid links found. Ensure the CSV has a "link" or "url" column, or put links in the first column.');
                return;
            }

            clearBulkError();
            populateBulkLinks(links);
            enterBulkMode();
        };
    ...
    $bulkList.on('click', '.btn-remove-bulk-link', function () {
        var $row = $(this).closest('.bulk-link-row');
        if ($bulkList.find('.bulk-link-row').length > 1) {
            $row.remove();
        } else {
            $row.find('.bulk-link-input').val('');
        }
        reindexBulkRows();
        updateBulkCount();
    });
    ...
    // Download sample CSV
    $(document).on('click', '#bulk-links-csv-download-sample', function (e) {
        e.preventDefault();
        var csv = 'link\nhttps://example.com/offer-1\nhttps://example.com/offer-2\nhttps://example.com/offer-3\n';
        var blob = new Blob([csv], { type: 'text/csv' });
        var a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = 'bulk-offer-links-sample.csv';
        a.click();
    });
    ...
    function enterBulkMode() {
        $singleWrapper.hide();
        $singleLinkInput.prop('disabled', true).prop('required', false);
        $bulkWrapper.show();
        if ($csvHint.length) $csvHint.show();
    }

    function populateBulkLinks(links) {
        $bulkList.empty();
        $.each(links, function (_i, url) { addBulkRow(url); });
        updateBulkCount();
    }

    function addBulkRow(url) {
        var index = $bulkList.find('.bulk-link-row').length;
        var $row = $(
            '<div class="bulk-link-row" data-index="' + index + '">' +
            '<span class="bulk-link-index">' + (index + 1) + '</span>' +
            '<input type="url" name="offer_bulk_links[]" value="' + escHtml(url) + '" class="regular-text bulk-link-input" placeholder="https://example.com/offer">' +
            '<button type="button" class="btn-remove-bulk-link" title="Remove">&times;</button>' +
            '</div>'
        );
        $bulkList.append($row);
        return $row;
    ...
    function reindexBulkRows() {
        $bulkList.find('.bulk-link-row').each(function (i) {
            $(this).attr('data-index', i).find('.bulk-link-index').text(i + 1);
        });
    }

    function updateBulkCount() {
        if (!$bulkCountLabel.length) return;
        var n = $bulkList.find('.bulk-link-row').length;
        $bulkCountLabel.text(n + ' link' + (n !== 1 ? 's' : ''));
    }

    function showBulkError(msg) { $csvError.text(msg).show(); }
    function clearBulkError()   { $csvError.text('').hide(); }
    ...
    function parseBulkCsv(text) {
        ...
        for (var i = 0; i < headers.length; i++) {
            if (headers[i] === 'link' || headers[i] === 'url') { linkCol = i; break; }
        }
        ...
        if (linkCol === -1) {
            ...
            linkCol  = 0;
            dataStart = isValidBulkUrl(firstCell) ? 0 : 1;
        }

        var links = [];
        for (var j = dataStart; j < lines.length; j++) {
            var cols = lines[j].split(',');
            var val  = (cols[linkCol] || '').replace(/['"]/g, '').trim();
            if (isValidBulkUrl(val)) links.push(val);
        }
        return links;
    }

    function isValidBulkUrl(val) {
        return /^https?:\/\/.+/.test(val);
    }

    // Called at the TOP of the form submit handler.
    // When bulk mode is active:
    //   - keep offer_bulk_links[] inputs enabled so they submit as an array
    //   - disable offer_link so it doesn't submit an empty/stale value
    function packBulkLinksIntoField() {
        if (!$bulkWrapper.is(':visible')) return;

        // Ensure all non-empty bulk rows are enabled for submission
        $bulkList.find('.bulk-link-input').each(function () {
            var val = $(this).val().trim();
            $(this).prop('disabled', !val); // disable only if empty (skip empties)
        });

        // Disable the single offer_link field — PHP will use offer_bulk_links[] instead
        $singleLinkInput.prop('disabled', true);
    }

    // ================================================================
    // END BULK OFFER LINKS
    // ================================================================
});
